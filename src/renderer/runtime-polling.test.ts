...
describe("runtime polling plan", () => {
  it("only requests full snapshots on the process page", () => {
    expect(runtimePollingPlan("processes", false, 0)).toEqual({ mode: "full", intervalMs: 1500 });
    expect(runtimePollingPlan("games", false, 0).mode).toBe("managed");
  });
  ...
  it("backs off in settings, while idle, and while hidden", () => {
    ...
    expect(runtimePollingPlan("processes", true, 0)).toEqual({ mode: "managed", intervalMs: 12_000 });
  });
