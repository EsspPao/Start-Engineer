...
import { buildThemeAttributes, buildWallpaperBackgroundStyle } from "./theme-attributes";
...
const defaultWallpaperBackground = { fit: "cover", focusX: 50, focusY: 50, dim: 18 } as const;
const emptyWallpaperBackground: WallpaperBackgroundState = { hasImage: false };
...
const fallbackApi: StartEngineerApi = {
  ...
  getPreferences: async () => ({ launchAtStartup: false, closeBehavior: "tray", globalShortcutEnabled: true, globalShortcut: "Ctrl+Shift+Space", uiTheme: "apple", wallpaperGlassIntensity: 55, wallpaperGlassVariant: "dark", wallpaperBackground: defaultWallpaperBackground, runAsAdministrator: false, searchProvider: "everything", sortRunningAppsFirst: true, showAppNames: false, keyboardShortcuts: defaultKeyboardShortcuts, uiLayout: defaultUiLayoutPreferences, allAppsView: { orderedAppIds: [] }, firstRunImportCompleted: false, globalShortcutStatus: "registered", isRunningAsAdministrator: false, administratorStatusLoading: false, administratorRestartRequired: false }),
  updatePreferences: async (input) => ({ launchAtStartup: input.launchAtStartup ?? false, closeBehavior: input.closeBehavior ?? "tray", globalShortcutEnabled: input.globalShortcutEnabled ?? true, globalShortcut: input.globalShortcut ?? "Ctrl+Shift+Space", uiTheme: input.uiTheme ?? "apple", wallpaperGlassIntensity: input.wallpaperGlassIntensity ?? 55, wallpaperGlassVariant: input.wallpaperGlassVariant ?? "dark", wallpaperBackground: input.wallpaperBackground ?? defaultWallpaperBackground, runAsAdministrator: input.runAsAdministrator ?? false, searchProvider: input.searchProvider ?? "everything", sortRunningAppsFirst: input.sortRunningAppsFirst ?? true, showAppNames: input.showAppNames ?? false, keyboardShortcuts: input.keyboardShortcuts ?? defaultKeyboardShortcuts, uiLayout: input.uiLayout ?? defaultUiLayoutPreferences, allAppsView: input.allAppsView ?? { orderedAppIds: [] }, firstRunImportCompleted: input.firstRunImportCompleted ?? false, everythingCliPath: input.everythingCliPath, globalShortcutStatus: "registered", isRunningAsAdministrator: false, administratorStatusLoading: false, administratorRestartRequired: Boolean(input.runAsAdministrator) }),
  getWallpaperBackground: async () => emptyWallpaperBackground,
  pickWallpaperBackground: async () => null,
  removeWallpaperBackground: async () => emptyWallpaperBackground,
...
function App() {
  ...
  const [preferences, setPreferences] = useState<AppPreferencesState>({ launchAtStartup: false, closeBehavior: "tray", globalShortcutEnabled: true, globalShortcut: "Ctrl+Shift+Space", uiTheme: "apple", wallpaperGlassIntensity: 55, wallpaperGlassVariant: "dark", wallpaperBackground: defaultWallpaperBackground, runAsAdministrator: false, searchProvider: "everything", sortRunningAppsFirst: true, showAppNames: false, keyboardShortcuts: defaultKeyboardShortcuts, uiLayout: defaultUiLayoutPreferences, allAppsView: { orderedAppIds: [] }, firstRunImportCompleted: false, globalShortcutStatus: "registered", isRunningAsAdministrator: false, administratorStatusLoading: false, administratorRestartRequired: false });
  const [wallpaperBackground, setWallpaperBackground] = useState<WallpaperBackgroundState>(emptyWallpaperBackground);
  ...
  const wallpaperBackgroundStyle = useMemo(() => buildWallpaperBackgroundStyle(preferences.wallpaperBackground, wallpaperBackground), [preferences.wallpaperBackground, wallpaperBackground]);
  ...
  useEffect(() => {
    ...
    void Promise.all([api().listGroups(), api().listApps(), api().listFolders(), api().listGroupGridOrders(), api().getPreferences()]).then(([nextGroups, nextApps, nextFolders, nextGridOrders, nextPreferences]) => {
      ...
      if (nextPreferences.uiTheme === "wallpaper") void api().getWallpaperBackground().then(setWallpaperBackground).catch(() => undefined);
  ...
  const saveTheme = useCallback(async (uiTheme: UiTheme) => {
    ...
    try {
      ...
      setPreferences(next);
      if (uiTheme === "wallpaper") void api().getWallpaperBackground().then(setWallpaperBackground).catch(() => undefined);
      return next;
    } catch (reason) {
      setPreferences(previous);
