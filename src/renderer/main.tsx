...
import { appDragPreviewStyle } from "./drag-preview-position";
...
function App() {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (command === "search" && activeSection !== "settings" && !hasModal && !isTextInputTarget(event.target)) { event.preventDefault(); searchInputRef.current?.focus(); return; }
