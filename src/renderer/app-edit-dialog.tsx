...
export type AppEditState = { id: string; name: string; executablePath: string; launchArgs: string; appUserModelId?: string } | null;
...
export function AppEditDialog({ state, onClose, onPickExecutable, onSave }: { state: AppEditState; onClose: () => void; onPickExecutable: (id: string) => Promise<string | null>; onSave: (input: UpdateAppInput) => Promise<void> }) {
  ...
  const chooseExecutable = async () => {
    ...
    try {
      ...
      if (executablePath) setForm((current) => current ? { ...current, executablePath, appUserModelId: undefined } : current);
    } catch (reason) {
  ...
  const save = async () => {
    ...
    setBusy(true);
    await onSave({ id: form.id, name: form.name.trim(), executablePath: form.executablePath, launchArgs: form.launchArgs.trim() || undefined, appUserModelId: form.appUserModelId });
    onClose();
  };
  return <div className="modal-backdrop no-drag" onPointerDown={onClose}><form className="dialog edit-dialog" onSubmit={(event) => { event.preventDefault(); void save(); }} onPointerDown={(event) => event.stopPropagation()}><h2>编辑应用信息</h2><label>名称<input autoFocus value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label><label>{form.appUserModelId ? "Windows 应用标识" : "启动程序"}<span className="executable-picker"><input value={form.appUserModelId || form.executablePath} readOnly title={form.appUserModelId || form.executablePath} /><button type="button" className="ghost" onClick={() => void chooseExecutable()} disabled={busy || selectingProgram}>{selectingProgram ? "选择中..." : form.appUserModelId ? "改用本地程序" : "选择程序"}</button></span></label><label>启动参数<input value={form.launchArgs} onChange={(event) => setForm({ ...form, launchArgs: event.target.value })} placeholder="例如：--silent" /></label>{selectionError ? <p className="dialog-error">{selectionError}</p> : null}<div className="dialog-actions"><button type="button" className="ghost" onClick={onClose} disabled={busy || selectingProgram}>取消</button><button type="submit" className="launch" disabled={busy || selectingProgram || !form.name.trim() || (!form.executablePath && !form.appUserModelId)}>保存</button></div></form></div>;
}
