    ...
    <section className="content group-content no-drag">
      <div className="app-grid" tabIndex={-1}>
        ...
        {apps.length ? apps.map((app) => {
          ...
          return (
            <div key={app.id} data-app-card-id={app.id} className={`app-card-wrap ${showAppNames ? "" : "names-hidden"} ${app.metrics.isRunning ? "running" : ""} ${isCurrent ? "current" : ""} ${isLaunching ? "launching" : ""} ${isInvalid ? "invalid" : ""} ${app.id === draggingAppId ? "drag-placeholder" : ""}`}>
              <button
...
export const UnifiedGroupPage = memo(function UnifiedGroupPage(props: UnifiedGroupPageProps) {
  ...
  const appCard = (app: RuntimeApp, sourceFolderId?: string) => {
    ...
    return <div key={`${sourceFolderId ?? "outer"}-${app.id}`} data-app-card-id={app.id} data-grid-item-id={sourceFolderId ? undefined : itemId} data-folder-member-id={sourceFolderId ? app.id : undefined} className={`app-card-wrap ${showAppNames ? "" : "names-hidden"} ${app.metrics.isRunning ? "running" : ""} ${isCurrent ? "current" : ""} ${isLaunching ? "launching" : ""} ${isInvalid ? "invalid" : ""} ${draggingItemId === itemId ? "drag-placeholder" : ""}`}>
      <button className={`app-card ${showAppNames ? "" : "names-hidden"} ${isCurrent ? "current" : ""} ${isLaunching ? "launching" : ""}`} title={app.name} aria-label={app.name} onClick={(event) => { if (event.detail === 1) activate(app, "single"); }} onDoubleClick={() => activate(app, "double")} onContextMenu={(event) => props.onContextMenu(event, app)} onPointerDown={(event) => props.onAppPointerDown(event, app, sourceFolderId)}>
  ...
  for (const itemId of renderItemOrder) {
    ...
    renderedItems.push(
      <div key={folder.id} data-grid-item-id={itemId} data-folder-id={folder.id} className={["app-card-wrap", "folder-card-wrap", folderIsRunning && "running", folderIsPartiallyRunning && "partial-running", selectedItemId === itemId && "current", folderIsLaunching && "folder-batch-active", draggingItemId === itemId && "drag-placeholder"].filter(Boolean).join(" ")}>
