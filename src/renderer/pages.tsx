...
type UnifiedGroupPageProps = {
  ...
  onToggleLaunchSelected: (app: RuntimeApp) => void;
...
export const UnifiedGroupPage = memo(function UnifiedGroupPage(props: UnifiedGroupPageProps) {
  const { apps, allApps, folders, itemOrder, expandedFolderId, launchingAppIds, folderLaunchStatuses = {}, selectedAppId, invalidAppIds, draggingItemId, selectedCount, runningCount, showAppNames } = props;
  ...
  const appCard = (app: RuntimeApp, sourceFolderId?: string) => {
    ...
    return <div key={`${sourceFolderId ?? "outer"}-${app.id}`} data-app-card-id={app.id} data-grid-item-id={sourceFolderId ? undefined : itemId} data-folder-member-id={sourceFolderId ? app.id : undefined} className={`app-card-wrap ${showAppNames ? "" : "names-hidden"} ${app.metrics.isRunning ? "running" : ""} ${app.launchSelected ? "launch-selected" : ""} ${isCurrent ? "current" : ""} ${isLaunching ? "launching" : ""} ${isInvalid ? "invalid" : ""} ${draggingItemId === itemId ? "drag-placeholder" : ""}`}>
      <button className={`app-card ${showAppNames ? "" : "names-hidden"} ${app.launchSelected ? "launch-selected" : ""} ${isCurrent ? "current" : ""} ${isLaunching ? "launching" : ""}`} title={app.name} aria-label={app.name} onClick={(event) => { if (event.detail === 1) activate(app, "single"); }} onDoubleClick={() => activate(app, "double")} onContextMenu={(event) => props.onContextMenu(event, app)} onPointerDown={(event) => props.onAppPointerDown(event, app, sourceFolderId)}>
        {isInvalid ? <span className="invalid-path-badge">!</span> : null}<div className="card-icon">{app.iconDataUrl ? <img src={app.iconDataUrl} draggable={false} alt="" /> : <GridIcon />}</div>{showAppNames ? <span className="app-name">{app.name}</span> : null}{isLaunching ? <span className="launching-overlay"><i />启动中</span> : null}
      </button>
      <button type="button" className="app-check" aria-pressed={Boolean(app.launchSelected)} onPointerDown={(event) => event.stopPropagation()} onClick={(event) => { event.stopPropagation(); props.onToggleLaunchSelected(app); }}>{app.launchSelected ? "✓" : ""}</button>
  ...
  for (const itemId of renderItemOrder) {
    ...
    renderedItems.push(
      <div key={folder.id} data-grid-item-id={itemId} data-folder-id={folder.id} className={`app-card-wrap folder-card-wrap ${folderIsLaunching ? "folder-batch-active" : ""} ${draggingItemId === itemId ? "drag-placeholder" : ""}`}>
        <button className={`app-card folder-card ${showAppNames ? "" : "names-hidden"} ${renderedFolderId === folder.id ? "folder-source-hidden" : ""}`} onClick={(event) => { if (event.detail !== 1 || document.documentElement.dataset.cardDragging) return; const origin = event.currentTarget.getBoundingClientRect(); if (folderClickTimer.current) window.clearTimeout(folderClickTimer.current); folderClickTimer.current = window.setTimeout(() => { folderClickTimer.current = null; setFolderOrigin(origin); props.onToggleFolder(folder.id); }, 220); }} onDoubleClick={(event) => { event.preventDefault(); event.stopPropagation(); if (folderClickTimer.current) { window.clearTimeout(folderClickTimer.current); folderClickTimer.current = null; } if (!document.documentElement.dataset.cardDragging) props.onLaunchFolder(folder.id); }} onPointerDown={(event) => props.onFolderPointerDown(event, folder)}>
          <div className="folder-icon" style={{ "--folder-columns": columns, "--folder-icon-size": `${iconSize}px` } as React.CSSProperties}>
            {members.map((app) => {
              const status = folderLaunchStatuses[app.id];
              return <span key={app.id} className={`folder-member-launch ${status ?? ""}`}>{app.iconDataUrl ? <img src={app.iconDataUrl} draggable={false} alt="" /> : <GridIcon />}<i aria-hidden="true" /></span>;
            })}
          </div>
