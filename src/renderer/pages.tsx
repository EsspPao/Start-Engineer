...
export const ProcessPage = memo(function ProcessPage({ processes, lockedProcessName, sortKey, sortDirection, changeSort, filter, setFilter, onContextMenu }: { processes: DisplayProcess[]; lockedProcessName: string; sortKey: SortKey; sortDirection: "asc" | "desc"; changeSort: (key: SortKey) => void; filter: ProcessFilter; setFilter: (value: ProcessFilter) => void; onContextMenu: (event: React.MouseEvent, process: ProcessInfo) => void }) {
  ...
  return <section className="content no-drag"><div className="table-toolbar"><div className="segmented"><button className={filter === "all" ? "selected" : ""} onClick={() => setFilter("all")}>全部进程</button><button className={filter === "managed" ? "selected" : ""} onClick={() => setFilter("managed")}>已管理应用</button></div></div><div ref={tableRef} className="process-table" onScroll={(event) => setViewport({ top: event.currentTarget.scrollTop, height: event.currentTarget.clientHeight })}>
    ...
    <div className="process-virtual-body" style={{ height: processes.length * rowHeight }}>{visible.map((process, index) => <div style={{ transform: `translateY(${(start + index) * rowHeight}px)` }} className={`process-row virtual ${process.name.toLowerCase() === lockedProcessName ? "locked" : ""} ${process.isEnded ? "ended" : ""}`} key={process.name.toLowerCase()} onContextMenu={(event) => onContextMenu(event, process)}><div className="process-icon">{process.iconDataUrl ? <img src={process.iconDataUrl} alt="" /> : initials(process.name)}</div><div><p>{process.name}</p><span title={`PID: ${process.pids.join(", ")}`}>{process.isEnded ? "已结束" : process.processCount > 1 ? `${process.processCount} 个进程` : `PID ${process.pid}`}</span></div><span>{process.isEnded ? "--" : `${process.cpuPercent.toFixed(1)}%`}</span><span>{process.isEnded ? "--" : formatMemory(process.memoryBytes)}</span><span>{process.isEnded ? "--" : formatDisk(process.diskBytesPerSecond)}</span></div>)}</div>
  </div></section>;
});

export const GroupPage = memo(function GroupPage({ apps, selectedAppId, draggingAppId, onSelect, onLaunch, onAdd, onPickExecutable, onContextMenu, onPointerDown, onRequestClose }: { apps: RuntimeApp[]; selectedAppId: string; draggingAppId?: string; onSelect: (id: string) => void; onLaunch: () => void; onAdd: () => void; onPickExecutable: () => void; onContextMenu: (event: React.MouseEvent, app: RuntimeApp) => void; onPointerDown: (event: React.PointerEvent, app: RuntimeApp) => void; onRequestClose: (app: RuntimeApp) => void }) {
  const selected = apps.find((app) => app.id === selectedAppId);
  return <section className="content group-content no-drag"><div className="app-grid">{apps.map((app) => <div key={app.id} className={`app-card-wrap ${app.metrics.isRunning ? "running" : ""} ${app.id === selectedAppId ? "selected" : ""}`}><button className={`app-card ${app.id === selectedAppId ? "selected" : ""} ${app.id === draggingAppId ? "dragging" : ""}`} onClick={() => !draggingAppId && onSelect(app.id)} onContextMenu={(event) => onContextMenu(event, app)} onPointerDown={(event) => onPointerDown(event, app)}><div className="card-icon">{app.iconDataUrl ? <img src={app.iconDataUrl} draggable={false} alt="" /> : <GridIcon />}</div><span title={app.name}>{app.name}</span></button>{app.metrics.isRunning ? <button className="app-close" title={`结束 ${app.name}`} aria-label={`结束 ${app.name}`} onPointerDown={(event) => event.stopPropagation()} onClick={(event) => { event.preventDefault(); event.stopPropagation(); onRequestClose(app); }}>×</button> : null}</div>)}</div><div className="group-actions"><button className="ghost" onClick={onAdd}>添加应用</button><button className="ghost" onClick={onPickExecutable} disabled={!selected}>选择程序</button><button className="launch" onClick={onLaunch} disabled={!selected}><PlayIcon />启动</button></div></section>;
});
