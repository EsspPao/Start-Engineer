...
- 用户手动调整后确认默认窗口为 1440 × 800 逻辑像素，替代此前 1200 × 720。默认预览及尺寸回归同步更新；小屏幕继续按可用区域缩小，避免越界。
- 当前个人窗口记录同步至 1440 × 800，保留位置且写入前备份。513 项测试、构建、启动与真实编辑器回归通过；本地安装版、便携版和校验文件已更新，未发布远程。

## 启动窗口尺寸收紧（2026-09-20）

- 默认改为居中的 1200 × 720 逻辑像素；启动尺寸最多占当前显示器工作区的 90%，小屏幕最小尺寸同步缩小，避免 Electron 的最小尺寸把窗口撑回全屏。
- 旧 1461 × 810 默认值及超过工作区 90% 的旧尺寸记录恢复到新默认；正常手动尺寸与多屏位置保留。外观工作台默认预览同步为 1200 × 720。
- 验证：513 项测试、类型检查、构建、隔离启动及真实窗口/编辑器回归通过；本地安装包及 SHA-256 已更新，未推送或发布远程。

## 安装后搜索刷新（2026-09-20）

- 快捷方式扫描不再无限缓存；只合并正在进行的并发扫描，后续搜索重新读取开始菜单和桌面。可添加应用与已安装下载过滤共享本轮扫描。
- 返回启动台窗口、重新聚焦已有关键词的搜索框会刷新结果，不需要修改关键词或重启；清空查询及刷新时作废旧请求，防止异步结果回填。
- 新增安装、卸载、扫描失败恢复回归，以及 scripts/verify-search-refresh.mjs 隔离界面回归，验证同一关键词从下载入口切换为可添加结果。
- 验证：106 个测试文件、512 项测试、类型检查、生产构建、启动及真实窗口焦点切换回归通过；本地安装版、便携版和校验文件已更新。未推送或发布远程。

## 官方下载目录第二批扩充（2026-09-19）

- 从 30 款扩充至 49 款：UU加速器、UU远程、网易云音乐、QQ音乐、WPS Office、百度网盘、GOG GALAXY、LibreOffice、GIMP、Krita、Audacity、qBittorrent、Docker Desktop、PyCharm、Sublime Text、WinSCP、Zotero、SumatraPDF、HandBrake。
- 网易 UU 加速器官网页面指向 /download/，实际读取返回 200 和“网易UU加速器_最新版下载”；UU远程使用 uuyc.163.com/download/。不采纳搜索结果中的近似域名。其余目录使用实际读取或官方网页检索核对的 HTTPS 官方入口。
- UU加速器关联 uu_launcher，UU远程关联 GameViewer；回归覆盖两款 UU 产品、QQ 与 QQ音乐相互独立的已安装过滤。官网入口仅打开网页，不直接运行安装包。
- 验证：105 个测试文件、509 项测试、类型检查、生产构建与隔离启动通过。本地 release 安装版、便携版和校验文件已更新；尚未推送或发布远程。

## 官方下载目录扩充（2026-09-19）

- 从 10 款扩充到 30 款：新增 Snipaste、7-Zip、Everything、OBS Studio、VLC、Firefox、Edge、Brave、Figma、Discord、Telegram、Blender、飞书、钉钉、腾讯会议、IntelliJ IDEA、Python、Node.js、GitHub Desktop、Postman。
- 逐项核对官方页面或官方站点检索结果；入口保持 HTTPS 官方下载页，不采用第三方下载站。VLC 使用 VideoLAN 官方 images.videolan.org 下载页；部分页面动态加载，未下载或执行第三方安装包。
- 添加中英文别名和常见 Windows 进程名以支持搜索与已安装过滤；保留短关键词去噪和本机已有应用隐藏规则。当前仍由浏览器打开下载页。
- 验证：105 个测试文件、503 项测试、类型检查、构建及隔离启动通过；release 根目录安装版、便携版及校验文件已更新。未推送远程或发布 GitHub Release。

## 短关键词搜索去噪（2026-09-19）

- 一至两个字符只匹配应用显示名称；可安装目录同时允许明确别名，不匹配发布者或描述。三个及以上字符保留进程名搜索。共享规则同时用于已添加应用和本机候选，不针对特定字母或系统应用设黑名单。
- 修复 hu / ui 因 FeedbackHub、SecHealthUI 内部进程名而返回反馈中心和 Windows 安全中心；保留中文、英文、数字名称匹配、全角归一化及大小写忽略。
- 验证：105 个测试文件、494 项测试、类型检查、生产构建和隔离启动通过；本地安装版、便携版及校验文件已更新，未发布远程。

## 搜索相关性与已安装应用过滤（2026-09-19）

- 已添加应用与本机候选统一按名称精确、前缀、包含，再按进程名精确、前缀、包含排序；同等相关性才考虑来源。应用搜索不再匹配目录路径或 Store AUMID，避免单字母命中 Users / WindowsApps 等路径造成大量无关结果。
- 可安装目录按本机开始菜单、桌面、Store、Everything 和已有应用的名称/进程身份过滤；兼容目录内中英文别名与全角字符，保留 Notion Calendar 等不同产品的独立下载入口。
- 本机普通候选在展示前验证目标文件存在，避免失效快捷方式显示为可添加应用。未修改应用分组中的用户排列顺序。
- 验证：486 项测试、类型检查、生产构建和隔离启动通过；已更新 release 根目录安装版、便携版及 SHA-256 校验文件。本次未推送或更新 GitHub Release。

## 搜索去重与界面布局编辑优化（2026-09-08）

- 搜索候选不再只按 EXE 路径 / AUMID 分别展示：名称与进程身份一致的 Microsoft Store、开始菜单、桌面和 Everything 候选会合并为一个结果，优先保留已添加状态，其次保留质量更高的发现来源。已经出现在“已添加应用”区域的候选不会再次出现在“本机可添加应用”，修复 Snipaste 搜索出现多个同名条目的问题。
- 设置页“界面编辑器”改为更直接的“界面布局”：删除与真实页面不一致的缩略模型和滚轮缩放入口，明确所有修改实时作用于当前窗口；比例与背景作为首要控件，其他选项按“尺寸与间距”“显示内容”分组，分享码降级到按需展开的“导入与分享”。
- 界面布局编辑新增一键“恢复默认”，同时恢复比例、背景、卡片、网格、侧栏、品牌图标和显示开关；按钮在已经使用默认值或正在保存时禁用。
- 回归测试新增跨 Store / 开始菜单 / Everything 的 Snipaste 身份合并，以及渲染层隐藏已添加候选的断言。验证：`npm run typecheck` 通过；完整 Vitest 共 102 个测试文件 / 471 项测试通过；生产构建、helper Smoke（195 个快捷方式）与 Electron 隔离启动 Smoke 通过。已重新生成 `release/Start-Engineer-Setup-0.1.4.exe`（82.97 MiB）和 `release/Start-Engineer-Portable-0.1.4.exe`（82.73 MiB），发布体积检查通过；SHA-256 分别为 `27375368397bc085a991f93c134a1cda7229550d3af68aff8b6e51beef72ea63`、`033bb557efe19c8d27fb666a8f00447a553e4b217f984a921a7dcc2affc1dcbe`。
- 使用重新打包的 Portable 成品做 Windows 实机检查：搜索 Snipaste 只显示一个已添加结果，未再出现 Store、开始菜单与本地扫描的重复候选；“界面布局”可进入编辑状态，比例、背景颜色、尺寸与间距分组、恢复默认和完成按钮均正常显示。实机截图透出聊天背景且存在裁切，提交前已撤下文章引用，仅保留在本地，不上传公开仓库。布局控件显示经过目视检查，未完成所有交互路径验收。

## 小黑盒图文稿更新（2026-09-08）

- 用户已明确恢复文章任务，并授权操作已打开的 Start Engineer 截图。下方“新对话交接”保留为此前暂停时的历史记录，以本段为最新进度。
- `docs/XIAOHEIHE_INTRO_DRAFT.md` 已更新为 v0.1.4 简洁图文稿，嵌入 `docs/assets/xiaoheihe/` 下两张干净的真实界面截图：游戏分组、多应用卡片展开；搜索与布局说明保留文字，配图待重新拍摄。原有配图占位和 v0.1.3 链接已移除。
- 截图通过 Windows Computer Use 的窗口捕获直接保存，未使用生成图或修改产品界面像素。玻璃窗口原先透出聊天内容，因此临时使用应用自带的 `#F5F5F7` 背景拍摄；外观配图是主题选择入口，不是 Wallpaper Glass / Clear Desktop 壁纸效果对比，正文已注明自定义背景。
- 本轮只导航、展开卡片、搜索和临时调整背景；没有启动、关闭、增删应用或修改应用分组。截图完成后恢复跟随主题背景，保留 Wallpaper Glass 浅色、融合强度 0% 和界面比例 100%。
- 版本说明以本地 package / v0.1.4 发布说明及已有发布验证记录核对；本轮网页读取发布页返回 Cache miss，未声称重新验证远程下载。仅文档与图片变更，不重打包、不提交、不推送、不发布小黑盒文章。

## 新对话交接（2026-09-08）

- **当前任务已暂停**：用户要求停止补充小黑盒文章，先更新本文档，准备在新对话继续。不要自动发布文章或重新发布 Release。
- **仓库基线**：本次交接前工作区干净，当前为 `main`，HEAD 为 `06ccc8e`（`Document verified v0.1.4 prerelease`），本地记录与 `origin/main` 一致；本轮未重新联网 fetch。本轮仅修改本文档，尚未提交或推送。
- **发布已完成**：v0.1.4 已作为 GitHub Pre-release 公开，地址为 https://github.com/EsspPao/Start-Engineer/releases/tag/v0.1.4 。安装版、便携版下载校验和隔离启动验证已完成，详细结果见第 13 节；交互式安装 / 卸载验证仍未执行。不要把旧对话中的“继续发布”误当成待执行任务。
- **文章现状**：已有 `docs/XIAOHEIHE_INTRO_DRAFT.md`，但仍是旧版文字草稿，包含配图占位和 v0.1.3 下载链接，尚未升级为完整图文稿。下一轮应基于实际最新版本核对内容，而非直接发布该文件。
- **用户期望**：简洁明了的完整小黑盒介绍，文字少、真实功能配图清晰；可以直接操作 Start Engineer 截图。先在 Markdown 文件中交付审阅，未经明确授权不得发到小黑盒。重点可展示分组整理、多应用卡片展开、搜索添加、Wallpaper Glass / Clear Desktop 外观。
- **本轮实际进度**：只检查了旧草稿、仓库状态，并读取电脑操作技能及其 guidance / API / confirmations 文档，确认有 `node_repl` + `@oai/sky` 的 Windows 操作入口；尚未初始化截图会话、操作应用、生成截图或修改文章。新对话需按当时可用技能重新读取操作说明并重新枚举窗口，不要假定已有截图或可复用窗口句柄。
- **配图边界**：使用真实界面，不用生成图冒充产品截图；避免暴露个人路径、聊天内容及桌面杂项。透明主题可能透出背后的窗口，应检查实际截图；保留个人应用配置，临时导航或主题切换后恢复。壁纸公开展示权需确认，优先使用有权展示的素材或中性背景。
- **介绍准确性**：不能宣传所有托盘应用都可唤醒、替代任务管理器或只有几 MB。说明当前为 Windows 10/11 x64 公开测试版、安装包未签名、部分托盘应用需手动唤起、暂无自动更新；批量关闭前提醒保存工作。两篇用户提供的小黑盒参考链接此前未成功读取正文，不得声称已参考其具体内容。
- **交付约定继续有效**：每次改动同步更新本文档；功能更新打包直接输出到 `release/` 根目录，不创建按修复命名的打包目录。仅文章 / 截图 / 文档更新无需重打应用包。需要关闭 Start Engineer 时请用户手动关闭，不得终止无关应用；提交、push、tag 和公开发布须有明确授权。

下一轮建议从读取 `AGENTS.md`、本交接段和旧文章草稿开始，先确认运行中的 Start Engineer 窗口，再截取少量干净的真实界面并完善 Markdown，最后交付本地文件供用户审阅。
...
- 当前包括 49 款常见应用，覆盖工具、办公、聊天、浏览器、游戏与开发；新增目录详见本文官方下载目录扩充记录。
...
### 11.11 2026-09-06 v0.1.1 补丁版本发布

- 保留已经公开的 `v0.1.0` 标签与 Release，不移动、不覆盖旧版本产物；当前 `main` 以补丁版本 `0.1.1` 发布，确保源码提交、Git 标签、安装包文件名和应用内版本一致。
...
### 11.12 2026-09-06 修复分组排序回弹

- 分组拖动与右键上移 / 下移共用的保存入口仅调整数组顺序，未更新 `order`，配置归一化按旧编号排序导致回弹。现在先按请求顺序重新编号，再保存配置。
- 新增回归测试，验证返回顺序、磁盘持久化编号以及反向再次排序；沿用现有拖动动画与取消行为。
- 验证：分组服务 7 项测试、生产构建与隔离配置启动冒烟测试通过。本轮修复包输出到 `release/group-order-fix`，不覆盖 GitHub 已发布版本。

### 11.13 2026-09-06 简化空分组删除

- 2026-09-06：删除空分组时直接执行删除，不再展示“迁移 0 个应用”的弹窗；有应用（包括多应用卡片成员）的分组仍显示迁移确认。设置页与侧栏共用该逻辑，保留最后一个分组的保护和删除失败提示。
- 验证：分组服务和分组管理 14 项测试、生产构建、隔离配置启动冒烟测试通过；本地更新包位于 `release/group-delete-fix`。

### 11.14 2026-09-06 v0.1.2 发布与产物目录约定
...
2026-09-06：修复旧关联 PID 被无关进程复用导致绿灯误报的问题。运行监控与批量关闭状态识别共用进程身份校验，关联 PID 必须同时满足应用进程名 / 明确别名或已确认应用目录路径；路径缺失且名称不符时拒绝关联。快照采集清理失效会话关联，关闭应用前使用带路径的最新受管进程快照，保留合法同目录子进程支持。新增 Akari 被 steamservice 复用 PID、目录边界和无路径场景回归测试。
验证：101 个测试文件 / 468 项全量测试通过，随后新增的 RuntimeService 安全回归 1 项单独通过，确认失效关联被清除且不会调用普通 / 提权终止入口；类型检查、生产构建、helper 与隔离启动冒烟通过。重新生成本地 `release` 根目录安装版和便携版，未推送或更新 GitHub Release；本地修复包沿用 0.1.3 版本号，与线上旧包内容不同。

2026-09-06：新增根目录 `AGENTS.md` 贡献指南，概述模块结构、开发与验证命令、ES2020 / EditorConfig 约定、回归测试、提交要求及发布安全边界；明确功能改动打包到 `release` 根目录。本次仅更新文档，无运行时代码变更。
...
## 2026-09-09：窗口与运行状态规则

- 默认启动尺寸采用当前保存的实际窗口大小 1461 × 810 逻辑像素，启动时按显示器工作区限制大小和位置，小屏幕同步降低最小窗口尺寸。已有用户仍优先恢复保存的窗口。
- 移除运行应用置顶开关；已添加应用固定运行优先，自定义分组保留用户顺序。
- 移除显示运行状态开关，旧配置和分享码统一规范为显示状态。
- 验证：103 个测试文件、474 项测试通过，类型检查、生产构建及隔离启动通过。

## 2026-09-09：独立外观工作台（codex/appearance-studio）

- 前三项基础行为修改单独保存在 main 提交 ca22a11，随后建立独立分支重做界面自定义。小黑盒文章不在本次修改范围。
- 外观入口改为“自定义与分享”，以风格、布局、分享三个步骤组织独立模态工作台。草稿即时预览，显式保存/取消；支持撤销、重做、紧凑/舒展预设及默认窗口/小屏幕预览。
- 预览通过 Shadow DOM 复用生产主题 CSS 和 GroupPage 卡片，隔离主题与尺寸，不写入个人配置。预览应用不可执行操作；键盘焦点保持在编辑控件，原生分组快捷键不能切走工作台。
- 新增 seui:v2 完整外观分享码：主题、玻璃明暗及通透程度、背景颜色与氛围、比例、卡片与导航布局、显示内容。仅白名单视觉字段，不携带应用、路径、窗口位置、快捷键或权限。旧 seui:v1 布局码继续兼容并保留接收者主题；非法、超长、非对象内容返回错误，不保存。
- 导入先加载到草稿预览，保存时通过现有受限偏好 IPC 一次应用完整外观；分享码导入服务同步应用主题。
- 增加 scripts/verify-appearance-editor.mjs：在临时独立配置中验证生产界面的预览、撤销/重做、取消、非法分享码、完整导入/保存、原生快捷键隔离、1024×600 适配；运行方式 node_modules/.bin/electron scripts/verify-appearance-editor.mjs。
- 已完成真实 Windows 界面检查与 Electron 端到端回归；最终版本通过 104 个测试文件、479 项测试、类型检查、生产构建及隔离启动验证。安装版/便携版更新至 release 根目录；分支尚未合并或推送远程。

## 2026-09-15：修复预览悬停空白

- 复现原因为预览透明点击层命中通用按钮 hover 背景规则，较高选择器优先级让浅色背景遮住整个预览。将预览点击层排除出该规则，保留点击与键盘焦点行为。
- Electron 回归脚本新增真实 mouseMove 输入与 :hover 状态验证，检查覆盖层计算背景始终透明；修复前重现 rgb(240, 243, 255) 不透明背景导致断言失败。
- 修复后真实悬停断言、完整编辑器流程、479 项测试、类型检查、构建和隔离启动全部通过。

## 2026-09-15：拖动预览避让分组名称

- 应用和多应用卡片的拖动预览统一使用实际侧栏边界定位，靠近侧栏时保持在其右侧并留出 12 逻辑像素间隔，经过分组间隙时也不遮住名称。
- 鼠标目标命中逻辑不变，离开侧栏后恢复原抓取偏移；按界面缩放换算预览位置及尺寸。新增 80%、100%、125% 缩放与网格内拖动回归覆盖。
- 实际 Electron 输入回归在 125% 缩放下验证卡片不遮挡侧栏、目标分组保持高亮、松手后分组变更持久化。484 项测试、类型检查、构建与隔离启动通过；合并反馈偏移同步使用相同缩放坐标。

## 2026-09-17：分组接纳缩小动画

- 撤销侧栏边界限制，卡片随鼠标进入分组。命中时以 200ms 动画缩小到分组行右侧 36px 缩略卡片，名称保留独立空间；离开恢复大小，松手移动。支持缩放和减少动态效果。
- 回归验证覆盖 125% 缩放下进入分组缩小、名称可见、移回起点恢复、再次进入及松手持久化；同时修复拖动开始阈值错误拦截已开始拖动的起点更新。483 项测试、类型检查、构建及启动验证通过。

## 2026-09-20：辅助程序 UAC 品牌标识

- 原生辅助程序更名为 `Start-Engineer-Helper.exe`，文件说明使用“Start Engineer 辅助程序”，产品名称保留 Start Engineer，并嵌入应用图标；即使 Windows 回退显示文件名，也能识别所属应用。
- 同步开发环境、安装包中的调用路径及构建、冒烟和体积校验脚本。权限申请条件、普通启动权限和进程关闭校验保持原有行为。
- 验证：106 个测试文件 / 513 项测试、类型检查、生产构建、原生 helper 冒烟（196 个快捷方式）和 Electron 隔离启动通过；检查生成 EXE 的 Windows 版本资源，确认文件说明及产品名称。未在安全桌面实际触发 UAC 人工确认。
- 本地 release 安装版、便携版及 SHA256SUMS 已更新；安装包内 helper 的品牌资源与冒烟检查通过。本轮未推送远程或更新 GitHub Release。

## 2026-09-20：WeGame 托盘唤醒复用会话授权

- 原因：WeGame 内置策略使用 self-launch，普通启动返回 740 后每次都单独 runas，导致重复 UAC。实机只读扫描确认主窗口隐藏；普通权限直接恢复仍返回 tray-hidden，因此保留应用自身唤醒策略。
- 已运行的 WeGame 需要提权唤醒时，复用现有会话级辅助程序；首次按需授权，同一 Start Engineer 会话后续唤醒和高权限关闭共享连接。退出 Start Engineer 后释放授权；不改变系统 UAC 设置。
- 新命令 wake-wegame 仅接收单个 PID：主进程先核对快照进程名及配置路径，辅助程序再检查进程存活、同 Windows 会话、WeGame 进程名及实际 EXE 名。启动路径由目标进程获取，不接受调用方路径或参数。普通启动成功、目标不匹配、用户取消时均不追加逐次 runas。
- 106 个测试文件 / 519 项测试、类型检查、生产构建、helper 冒烟和 Electron 隔离启动通过；回归覆盖普通唤醒、740 会话转交、取消、路径不匹配及已授权连接重复使用。未进行实际 UAC 授权后的连续唤醒人工验收。
- 本地 release 安装版、便携版及 SHA256SUMS 已更新，体积检查通过；未推送远程或更新 GitHub Release。

## 2026-09-23：外部关闭应用后的状态刷新

- 可见应用页统一每 1 秒采集受管状态，不再因 60 秒无交互退到 6 秒；隐藏界面保持 12 秒、设置页保持低频，避免后台持续高频采集。
- 返回窗口或恢复可见时强制刷新，跳过普通缓存；若已有请求正在读取，完成后立即补一次强制刷新。移除依赖 requestAnimationFrame 的启动步骤，统一定时器调度与清理，避免焦点事件与轮询产生重复计时。
- 本次实机检查未发现鸣潮残留进程，未将窗口消失直接等同于进程退出，也未改变进程归属或终止行为。
- 新增 scripts/verify-runtime-refresh.mjs：隔离配置、模拟运行状态，在实际 Electron 界面验证长时间无交互后的外部退出、旧请求在途时返回窗口，分别在 1.8 秒 / 0.7 秒阈值内清除绿灯；不启动或关闭实际游戏。
- 106 个测试文件 / 519 项测试、类型检查、生产构建、helper 与 Electron 启动检查通过；鸣潮真实 Alt+F4 退出后的残留进程时长尚未现场复现。
- 已更新本地 release 安装版、便携版及 SHA256SUMS，打包与体积检查通过；未提交、推送或更新 GitHub Release。

## 2026-09-25：公开体验前的反馈、配置保护与鸣潮退出状态

- 设置页加入“反馈问题”入口，打开带版本、构建编号和系统信息的 GitHub Issue 草稿；用户填写复现步骤。诊断信息不自动附带用户名、应用路径或个人配置，复制的关于信息也隐去配置目录。
- 新增完整配置备份与恢复：备份应用、分组、文件夹、排序、偏好及图标；记录构建编号，首次保护和检测到构建变化时自动备份，升级后的配置加载前完成备份。设置页提供手动备份、备份列表和恢复入口。恢复前先生成安全备份，重启时在配置加载前应用；校验内容和路径，恢复失败则回滚。
- 实机记录鸣潮 Alt+F4 退出时，游戏主体及 Client-Win64-Shipping 于 23:35:11.337 退出，而同目录 KRSDKExternal 持续至 23:35:12.662。运行灯不再将这个 SDK 尾随进程算作鸣潮仍在运行；进程 PID 仍可保留供其他生命周期操作。此规则限定鸣潮，其他应用仍按原有进程归属规则识别。
- 新增备份完整性、篡改/路径校验、恢复回滚、构建变化、反馈链接和鸣潮运行状态回归测试；跨 Electron 进程重启验证恢复，设置页在 1440×800 与 1024×600 实际渲染验证。全量 `npm run release:verify` 通过：108 个测试文件、525 项测试、类型检查、构建、helper 与隔离启动检查。本地安装版、便携版和 SHA256SUMS 已重新生成；本轮未推送或发布 GitHub Release。
- Windows Sandbox 干净环境安装、启动、便携版及卸载验收记录见 `docs/CLEAN_ENV_ACCEPTANCE_2026-09-25.md`；不能将代码级或模拟状态回归等同于用户实机鸣潮体验验证。
- 验收期间发现本地便携包在主机烟测中两次触发 Windows `0x80000003` 异常，第二次在旧实例完全关闭后仍复现；沙盒重试也未能完成启动。便携版与卸载验收当前未通过，不应把这次本地包视为可公开发布版本。已停止在用户桌面重试启动，等待隔离环境定位。
- 烟测就绪条件已从网页加载完成改为 React 实际挂载，退出延迟由 100 毫秒改为 1 秒，以避开过早关闭窗口/子进程的可疑时序。修正后 108 个文件 / 525 项测试、类型检查、构建与本地重打包通过；未再触发用户桌面启动，故 `0x80000003` 是否消失仍待隔离复验。新本地安装版 SHA-256 为 `1648037be3cf8067452f5fc878c0c15099252e1a9b8fcfd83a2a38d450d9e19d`，便携版为 `6a77bd577de0a92ec02155dbe54384414c4aaaabfcc26eebc99a07293e07efc7`。
